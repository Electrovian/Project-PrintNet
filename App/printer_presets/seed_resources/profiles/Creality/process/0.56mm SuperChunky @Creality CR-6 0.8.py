from __future__ import annotations

# source: profiles/Creality/process/0.56mm SuperChunky @Creality CR-6 0.8.json
DATA = {'compatible_printers': ['Creality CR-6 SE 0.8 nozzle', 'Creality CR-6 Max 0.8 nozzle'],
 'from': 'system',
 'inherits': 'fdm_process_creality_common',
 'initial_layer_line_width': '0.8',
 'initial_layer_print_height': '0.4',
 'inner_wall_line_width': '0.8',
 'instantiation': 'true',
 'internal_solid_infill_line_width': '0.8',
 'layer_height': '0.56',
 'line_width': '0.8',
 'name': '0.56mm SuperChunky @Creality CR-6 0.8',
 'outer_wall_line_width': '0.8',
 'setting_id': 'GP004',
 'sparse_infill_line_width': '0.8',
 'support_line_width': '0.8',
 'top_shell_layers': '4',
 'top_surface_line_width': '0.8',
 'type': 'process'}
