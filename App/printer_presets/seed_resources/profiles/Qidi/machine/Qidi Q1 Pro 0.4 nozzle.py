from __future__ import annotations

# source: profiles/Qidi/machine/Qidi Q1 Pro 0.4 nozzle.json
DATA = {'bed_exclude_area': ['25x245', '25x240', '115x240', '115x245', '208x245', '208x240', '245x240', '245x245', '208x245'],
 'change_filament_gcode': '',
 'default_filament_profile': ['Qidi Generic PLA @Qidi Q1 Pro 0.4 nozzle'],
 'default_print_profile': '0.20mm Standard @Qidi Q1 Pro',
 'deretraction_speed': ['0'],
 'extruder_clearance_height_to_lid': '120',
 'extruder_clearance_height_to_rod': '40',
 'extruder_clearance_radius': '70',
 'from': 'system',
 'gcode_flavor': 'klipper',
 'inherits': 'fdm_qidi_x3_common',
 'instantiation': 'true',
 'layer_change_gcode': ';BEFORE_LAYER_CHANGE\n;[layer_z]\nLOG_Z\nG92 E0\n',
 'machine_end_gcode': 'M141 S0\n'
                      'M104 S0\n'
                      'M140 S0\n'
                      'G1 E-3 F1800\n'
                      'G0 Z{min(max_print_height, max_layer_z + 3)} F600\n'
                      'G0 X0 Y0 F12000\n'
                      '{if max_layer_z < max_print_height / 2}G1 Z{max_print_height / 2 + 10} F600{else}G1 '
                      'Z{min(max_print_height, max_layer_z + 3)}{endif}',
 'machine_max_jerk_e': ['2'],
 'machine_max_jerk_x': ['8'],
 'machine_max_jerk_y': ['8'],
 'machine_max_jerk_z': ['3'],
 'machine_pause_gcode': 'M0',
 'machine_start_gcode': 'PRINT_START BED=[hot_plate_temp_initial_layer] HOTEND=[nozzle_temperature_initial_layer] '
                        'CHAMBER=[chamber_temperature]\n'
                        'M83\n'
                        'M140 S[hot_plate_temp_initial_layer]\n'
                        'M104 S[nozzle_temperature_initial_layer]\n'
                        'G4 P3000\n'
                        'G0 X{max((min(print_bed_max[0], first_layer_print_min[0] + 80) - 85),0)} '
                        'Y{max((min(print_bed_max[1] - 5, first_layer_print_min[1] + 80) - 85), 0)} Z5 F6000\n'
                        'G0 Z[initial_layer_print_height] F600\n'
                        'G1 E3 F1800\n'
                        'G1 X{(min(print_bed_max[0], first_layer_print_min[0] + 80))} E{85 * 0.5 * '
                        'initial_layer_print_height * nozzle_diameter[0]} F3000\n'
                        'G1 Y{max((min(print_bed_max[1] - 5, first_layer_print_min[1] + 80) - 85), 0) + 2} E{2 * 0.5 * '
                        'initial_layer_print_height * nozzle_diameter[0]} F3000\n'
                        'G1 X{max((min(print_bed_max[0], first_layer_print_min[0] + 80) - 85), 0)} E{85 * 0.5 * '
                        'initial_layer_print_height * nozzle_diameter[0]} F3000\n'
                        'G1 Y{max((min(print_bed_max[1] - 5, first_layer_print_min[1] + 80) - 85), 0) + 85} E{83 * 0.5 '
                        '* initial_layer_print_height * nozzle_diameter[0]} F3000\n'
                        'G1 X{max((min(print_bed_max[0], first_layer_print_min[0] + 80) - 85), 0) + 2} E{2 * 0.5 * '
                        'initial_layer_print_height * nozzle_diameter[0]} F3000\n'
                        'G1 Y{max((min(print_bed_max[1] - 5, first_layer_print_min[1] + 80) - 85), 0) + 3} E{82 * 0.5 '
                        '* initial_layer_print_height * nozzle_diameter[0]} F3000\n'
                        'G1 X{max((min(print_bed_max[0], first_layer_print_min[0] + 80) - 85), 0) + 3} Z0\n'
                        'G1 X{max((min(print_bed_max[0], first_layer_print_min[0] + 80) - 85), 0) + 6}\n'
                        'G1 Z1 F600\n',
 'max_layer_height': ['0.32'],
 'min_layer_height': ['0.08'],
 'name': 'Qidi Q1 Pro 0.4 nozzle',
 'nozzle_diameter': ['0.4'],
 'nozzle_type': 'hardened_steel',
 'printable_area': ['0x0', '245x0', '245x245', '0x245'],
 'printable_height': '240',
 'printer_model': 'Qidi Q1 Pro',
 'printer_settings_id': 'Qidi',
 'retract_before_wipe': ['0%'],
 'retract_length_toolchange': ['2'],
 'retraction_length': ['0.8'],
 'retraction_minimum_travel': ['1'],
 'setting_id': 'GM001',
 'single_extruder_multi_material': '1',
 'thumbnails': ['160x160', '112x112'],
 'thumbnails_format': 'PNG',
 'type': 'machine',
 'wipe_distance': ['2']}
