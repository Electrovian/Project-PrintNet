...
from connectors.octoprint import OctoPrintConnector  # noqa: E402
...
class OctoPrintConnectorTests(unittest.TestCase):
    def setUp(self):
        self.printer = {
            "name": "Octo Device",
    ...
    def test_connect_reports_missing_config(self):
        connector = OctoPrintConnector(session=_FakeSession([]))
        result = connector.connect({"name": "Octo Device"})
        self.assertFalse(result["ok"])
        self.assertEqual(result["state"], "missing_config")

    def test_connect_success_reads_version(self):
        session = _FakeSession([_FakeResponse(200, {"server": "OctoPrint", "api": "0.1.9"})])
        connector = OctoPrintConnector(session=session)
        result = connector.connect(self.printer)
        self.assertTrue(result["ok"])
        self.assertEqual(result["state"], "ready")
        self.assertEqual(result["server"], "OctoPrint")
        self.assertEqual(result["api_version"], "0.1.9")
        self.assertEqual(len(session.calls), 1)
        method, url, kwargs = session.calls[0]
        self.assertEqual(method, "GET")
        self.assertEqual(url, "http://localhost:5000/api/version")
    ...
    def test_start_print_uses_remote_path(self):
        ...
        self.assertEqual(url, "http://localhost:5000/api/files/local/uploads/demo.gcode")
