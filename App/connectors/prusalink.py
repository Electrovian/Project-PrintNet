...
class PrusaLinkConnector(PrinterConnector):
    """PrusaLink connector implementation using PrusaLink HTTP API endpoints."""
    ...
    def _base_url(self, printer: Mapping[str, Any]) -> str:
        value = str(printer.get("prusalink_url", "") or printer.get("prusa_url", "")).strip()
        if not value:
            raise InvalidPrinterConfigError("PRUSALINK_URL_REQUIRED: prusalink_url is required.")
        return value.rstrip("/")

    def _api_key(self, printer: Mapping[str, Any]) -> str:
        value = str(printer.get("prusalink_api_key", "") or printer.get("prusa_api_key", "")).strip()
        if not value:
            raise InvalidPrinterConfigError("PRUSALINK_API_KEY_REQUIRED: prusalink_api_key is required.")
        return value
    ...
    def _request(self, method: str, url: str, **kwargs: Any) -> requests.Response:
        ...
        try:
            ...
            raise ConnectorOperationError(f"PRUSALINK_REQUEST_FAILED: {exc}") from exc
        if int(response.status_code) >= 400:
            snippet = str(getattr(response, "text", "") or "").strip()
            if len(snippet) > 160:
                snippet = snippet[:160]
            raise ConnectorOperationError(f"PRUSALINK_HTTP_{response.status_code}: {snippet}")
        return response
    ...
    def _resolve_remote_path(self, payload: Mapping[str, Any], fallback_name: str) -> str:
        ...
        if isinstance(files, Mapping):
            ...
            if isinstance(local, Mapping):
                ...
                if name:
                    return name
        path_field = str(payload.get("path", "") or "").strip()
        if path_field:
            return path_field
    ...
    def connect(self, printer: Mapping[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "ok": True,
                "state": "ready",
                "server": str(payload.get("server", "PrusaLink")),
    ...
    def start_print(
        ...
        *,
        ...
        return {"ok": True, "state": "submitted", "remote_path": resolved, "message": "PrusaLink print started."}
